...
const A = (t, e = 0, s = 1) => t > s ? s : t < e ? e : t, m = (t, e = 0, s = Math.pow(10, e)) => Math.round(s * t) / s,
    Ve = ({h: t, s: e, v: s, a: i}) => {
        const o = (200 - e) * s / 100;
        return {
            h: m(t),
        ...
        const o = Math.max(t, e, s), r = o - Math.min(t, e, s),
            a = r ? o === t ? (e - s) / r : o === e ? 2 + (s - t) / r : 4 + (t - e) / r : 0;
        return {
            h: m(60 * (a < 0 ? a + 6 : a)),
            s: m(o ? r / o * 100 : 0),
            v: m(o / 255 * 100),
...
const Et = ':host{display:flex;flex-direction:column;position:relative;width:200px;height:200px;user-select:none;-webkit-user-select:none;cursor:default}:host([hidden]){display:none!important}[role=slider]{position:relative;touch-action:none;user-select:none;-webkit-user-select:none;outline:0}[role=slider]:last-child{border-radius:0 0 8px 8px}[part$=pointer]{position:absolute;z-index:1;box-sizing:border-box;width:28px;height:28px;display:flex;place-content:center center;transform:translate(-50%,-50%);background-color:#fff;border:2px solid #fff;border-radius:50%;box-shadow:0 2px 4px rgba(0,0,0,.2)}[part$=pointer]::after{content:"";width:100%;height:100%;border-radius:inherit;background-color:currentColor}[role=slider]:focus [part$=pointer]{transform:translate(-50%,-50%) scale(1.1)}',
    Pt = "[part=hue]{flex:0 0 24px;background:linear-gradient(to right,red 0,#ff0 17%,#0f0 33%,#0ff 50%,#00f 67%,#f0f 83%,red 100%)}[part=hue-pointer]{top:50%;z-index:2}",
    Nt = "[part=saturation]{flex-grow:1;border-color:transparent;border-bottom:12px solid #000;border-radius:8px 8px 0 0;background-image:linear-gradient(to top,#000,transparent),linear-gradient(to right,#fff,rgba(255,255,255,0));box-shadow:inset 0 0 0 1px rgba(0,0,0,.05)}[part=saturation-pointer]{z-index:3}",
    q = Symbol("same"), se = Symbol("color"), Se = Symbol("hsva"), oe = Symbol("update"), Ee = Symbol("parts"),
...
const ie = {
    ...
    __shouldAlignStartHorizontally(s, i) {
        ...
        const r = Math.min(window.innerWidth, document.documentElement.clientWidth),
            a = !i && this.horizontalAlign === "start" || i && this.horizontalAlign === "end";
    ...
    __shouldAlignStart(s, i, o, r, a, l, c) {
        const d = o - s[l ? c.end : c.start] - r[c.end], C = s[l ? c.start : c.end] - r[c.start], _ = a ? d : C,
            k = _ > (a ? C : d) || _ > i;
        return a === k;
    }
    ...
    __calculatePositionInOneDimension(s, i, o, r, a, l) {
        const c = l ? r.start : r.end, d = l ? r.end : r.start, C = parseFloat(a.style[c] || getComputedStyle(a)[c]),
            _ = this.__adjustBottomProperty(c, r, C), $ = i[l ? r.start : r.end] - s[o === l ? r.end : r.start],
            k = _ ? `${_}px` : `${C + $ * (l ? -1 : 1)}px`;
        return {
            [c]: k,
            [d]: ""
...
let w = class extends Be {
    ...
    renderPicker() {
        ...
        if ((e = this.context) != null && e.metadata) {
            const s = this.context.scope === u.local ? this.context.metadata.displayName : `All ${this.context.metadata.displayName}s`,
                i = n`<span class="component-type">${s}</span>`,
                o = this.context.scope === u.local ? Je(this.context.component) : null,
                r = o ? n` <span class="instance-name-quote">"</span><span class="instance-name">${o}</span
